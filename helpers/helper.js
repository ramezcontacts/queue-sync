...
class Helper {
  ...
  createMetaDataForSku = (attribute_types, childProduct) => {
    return new Promise(async (resolve, reject) => {
      ...
      if (childProduct.sku_cf_id === 2) {
        let metaInfo = this.createMetaFromStr(attribute_types, childProduct.sku_attributes);
        retObj = {
      ...
      retObj.metadata.productType = process.env.PRODUCT_CATEGORY;
      resolve(retObj);
    });
  }
  prepareJsonObjForManualImport = async (dbData, attribute_types) => {
    return new Promise(async (resolve, reject) => {
      let allSkuObj = {
        skuInput: [],
        priceInput: [],
      ...
      if (dbData.length > 0) {
        ...
        for (let childProduct of dbData) {
          ...
          allSkuObj.stockItemInput.push({
            stock_location_id: childProduct.cl_stock_location_id,
            sku_code: skuCODE,
  ...
  dataImportInCLForManualImport = (allSkuData) => {
  ...
  getSQLQryForManualImport(
    ...
    switch (productCategory) {
      ...
      case "EYECARE":
        skuQry = `SELECT DISTINCT 
        ChildProduct.id as sku_code, AppProduct.id as parent_id, AppProduct.product_classification_id as sku_cf_id, AppProduct.product_name as sku_name, AppProduct.has_valid_main_image_url as sku_image, AppProduct.short_desc as sku_desc, ListPrice.price AS sku_list_price, SellPrice.price AS sku_sell_price, GspPrice.price AS sku_gsp_price, COALESCE(ProductInventoryTotal.total_qty, 0) AS sku_stock,Manufacturer.id as manufacturer_id, Manufacturer.name, Manufacturer.is_use_msrp_pricing_eyeglasses, Manufacturer.is_use_msrp_pricing_sunglasses, Manufacturer.default_markup_readers, Availability.cl_stock_location_id 
        FROM products AS ChildProduct 
        INNER JOIN products AS AppProduct
        ON AppProduct.id = ChildProduct.grouping_product_id
        
        LEFT JOIN product_prices AS CostPrice
        ON CostPrice.product_id = AppProduct.id AND CostPrice.price_type_id = 3 
        AND CostPrice.is_deleted = 0 
        
        INNER JOIN manufacturers AS Manufacturer
        ON Manufacturer.id = AppProduct.manufacturer_id
        LEFT JOIN product_prices AS ListPrice
        ON ListPrice.product_id = AppProduct.id AND ListPrice.price_type_id = 1
        AND ListPrice.is_deleted = 0
        
        LEFT JOIN product_prices AS SellPrice
        ON SellPrice.product_id = AppProduct.id AND SellPrice.price_type_id = 2
        AND SellPrice.is_deleted = 0 
        
        LEFT JOIN product_prices AS GspPrice
        ON (GspPrice.product_id = AppProduct.id AND GspPrice.price_type_id = 4 AND GspPrice.is_deleted = 0)
        LEFT JOIN product_inventory_totals AS ProductInventoryTotal
        ON (ProductInventoryTotal.product_id = ChildProduct.id AND ProductInventoryTotal.product_inventory_location_id IS NULL)
      ...
      case "CONTACTLENS":
        skuQry = `SELECT DISTINCT 
        ChildProduct.id as sku_code, AppProduct.id as parent_id, AppProduct.product_classification_id as sku_cf_id, AppProduct.product_name as sku_name, AppProduct.has_valid_main_image_url as sku_image, AppProduct.short_desc as sku_desc, ListPrice.price AS sku_list_price, SellPrice.price AS sku_sell_price, GspPrice.price AS sku_gsp_price, COALESCE(ProductInventoryTotal.total_qty, 0) AS sku_stock, NULL AS sku_color, NULL AS sku_frame_size, NULL AS sku_power, (SELECT GROUP_CONCAT(CONCAT(product_shippable_setup_attribute_type_id, '::', value_varchar)) FROM product_shippable_to_attribute_type_values WHERE product_id = ChildProduct.id) AS sku_attributes, Manufacturer.default_markup_readers, Availability.cl_stock_location_id  
